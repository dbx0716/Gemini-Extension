...
自动协调多个 Gemini 机器人完成游戏素材生成工作流。**所有机器人名称均可在配置面板中自定义**，以下使用默认名称说明：
...
- 点击「机器人配置」展开配置面板
- 填写各机器人的 Gemini Gem URL（共 8 个配置项）
  - URL 格式：`https://gemini.google.com/gem/xxxx/xxxx`
  - 获取方式：打开 Gemini 聊天页面，从浏览器地址栏复制链接
  - 链接来源为账号中保存的 Gem
- **所有机器人名称均可自定义修改**，点击名称输入框即可修改
- 第 1 个机器人默认使用当前页面
- 点击「保存配置」
...
第二步运行过程中和完成后，均可对已完成的机器人单独重做：

- **运行中重做**：某个机器人完成后，底部会立刻出现对应的 🔄 重做按钮。点击后该机器人重新执行，完成后自动接力后续步骤继续运行。
- **完成后重做**：第二步全部完成后，所有机器人的重做按钮均可见。点击后仅重新执行该机器人，不会触发后续步骤。

#### 4.2.6 注意事项

**第一步：**

- 点击「开始第一步」后，扩展会自动获取第 1 个机器人的回复，然后**自动暂停**。用户需查看回复后手动点击 **▶ 继续**，才会发送给下一个机器人。
- 如果第 1 个机器人**未勾选**，扩展会暂停并显示空输入框，用户需**手动填写内容**后点击继续。
- 每个机器人旁有勾选框，未勾选的机器人会自动跳过。

**第二步：**

- 修改分类内容后，需点击「**保存修改**」再开始第二步，否则使用的是旧数据。
- 第二步**暂停后编辑再恢复**，扩展会从修改过的那条开始重新执行。例如：正在跑第 5 个场景时暂停 → 修改第 3 个场景 → 恢复 → 从第 3 个场景重新跑。素材、参考图同理。
- 跑到第 **8** 个机器人（参考图）时，会暂停等待用户**填写参考图数据**（至少填一条），填好后点击继续。
- 等待回复时有倒计时，可点击「**跳过**」不等计时结束。

**通用：**

- 所有机器人的勾选框控制是否执行该步骤，不勾选则自动跳过。
- 机器人名称和 URL 均可在配置面板中自定义。
...
function createTask(config) {
  const task = {
    ...
    bots: {
      ...
      canvasMaster: { ran: false, content: '', images: [] },
      bot2: { ran: false, content: '' },
      bot6: { ran: false, content: '' },
      bot3: { ran: false, scenes: [], sceneSetting: '' },
      bot4: { ran: false, materials: [], materialSetting: '' },
      bot5: { ran: false, character: '' },
      bot7: { ran: false, referenceImages: [] }
    }
  };
  // 保存到历史
